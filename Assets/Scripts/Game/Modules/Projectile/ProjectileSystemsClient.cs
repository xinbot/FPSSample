...
public class HandleProjectileSpawn : BaseComponentSystem
{
    ...
    protected override void OnCreateManager()
    {
        ...
        PredictedProjectileGroup = GetComponentGroup(typeof(ProjectileData), typeof(PredictedProjectile),
            ComponentType.Subtractive<DespawningEntity>());
        IncommingProjectileGroup =
...
//            var inProjectileEntity = inProjectile.GetComponent<GameObjectEntity>().Entity;
            ...
            if (!m_world.GetEntityManager().HasComponent<PredictedProjectile>(inProjectileEntity))
            {
                for (var i = 0; i < predictedProjectileEntities.Length; i++)
                {
                    ...
                    if (predictedProjetile.TypeId !=
                        inProjectileData.TypeId)
                        continue;
...
//            m_world.RequestDespawn(gameObject, PostUpdateCommands);
            ...
            if (ProjectileModuleClient.LogInfo.IntValue > 0)
                GameDebug.Log(string.Format(
                    "<color=red>Predicted projectile {0} destroyed as it was not verified. startTick:{1}]</color>",
                    entity, predictedEntity.startTick));
        }
    }
}
...
public class DespawnClientProjectiles : BaseComponentSystem
{
    ...
    protected override void OnCreateManager()
    {
        ...
        DespawningClientProjectileOwnerGroup =
            GetComponentGroup(typeof(ClientProjectileOwner), typeof(DespawningEntity));
    }
    ...
    protected override void OnUpdate()
    {
        ...
        var clientProjectileOwnerArray =
            DespawningClientProjectileOwnerGroup.GetComponentDataArray<ClientProjectileOwner>();
...
//        GameDebug.Log("Free pool:" + pool.poolIndex + " index:" + index + " before count:" + pool.freeList.Count);
    ...
    public Entity CreateClientProjectile(Entity projectileEntity)
    {
        ...
        // Create client projectile
        var pool = pools[projectileData.TypeId];
        ...
        if (ProjectileModuleClient.LogInfo.IntValue > 0)
            GameDebug.Log(string.Format("Creating clientprojectile {0} for projectile {1}]", clientProjectile,
                projectileEntity));
